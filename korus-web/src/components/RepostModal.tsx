...
export default function RepostModal({ isOpen, onClose, postId, postContent, postUser, onRepostSuccess, onSuccess }: RepostModalProps) {
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget && !isProcessing) onClose(); }}>
      <div ref={modalRef} className="modal-content bg-[#1e1e1e] border border-[#222222] rounded-2xl shadow-2xl max-w-lg w-full">
        {/* Modal Header */}
        ...
        <div className="flex gap-3 p-6 border-t border-[#222222]">
          <button
            onClick={onClose}
            disabled={isProcessing}
            className="flex-1 px-5 py-3 bg-white/[0.08] border border-[#2a2a2a] text-[#fafafa] font-semibold rounded-lg hover:bg-white/[0.12] duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Cancel
          </button>
          <button
