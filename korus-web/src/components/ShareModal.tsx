...
export default function ShareModal({ isOpen, onClose, postId, postContent, postUser }: ShareModalProps) {
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-[var(--color-overlay-background)] backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div ref={modalRef} className="modal-content bg-[var(--color-surface)] border border-[var(--color-border-light)] rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[var(--color-border-light)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="heading-2 text-[var(--color-text)] font-semibold">Share Post</h2>
              <p className="text-sm text-[var(--color-text-secondary)]">by {displayUser}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5">
          <div className="p-4 rounded-xl border-2" style={{ background: 'linear-gradient(90deg, color-mix(in srgb, var(--korus-primary) 10%, transparent), color-mix(in srgb, var(--korus-secondary) 10%, transparent))', borderColor: 'color-mix(in srgb, var(--korus-primary) 30%, transparent)' }}>
            ...
            <p className="text-[var(--color-text)] text-sm leading-relaxed line-clamp-3">
              {truncatedContent}
            </p>
          </div>
        </div>
        ...
        <div className="p-5 space-y-5">
          <div>
            <h3 className="label text-[var(--color-text)] mb-3">Share to</h3>
          ...
          {/* Direct Link */}
          <div>
            <h4 className="label text-[var(--color-text)] mb-3">Direct Link</h4>
            <div className="flex items-center gap-2">
              <input
                type="text"
                value={postUrl}
                readOnly
                className="flex-1 bg-white/[0.06] text-[var(--color-text-secondary)] text-sm px-3 py-2.5 rounded-lg border border-[var(--color-border-light)] focus:outline-none"
              />
              <button
                onClick={async () => {
                  try {
