...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ variant = 'default', label, helperText, fullWidth = false, className = '', ...props }, ref) => {
    const baseStyles = 'bg-white/[0.06] text-[#fafafa] px-4 py-3 rounded-lg border border-white/15 transition-all duration-150';
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ variant = 'default', label, helperText, fullWidth = false, className = '', rows = 4, ...props }, ref) => {
    const baseStyles = 'bg-white/[0.06] text-[#fafafa] px-4 py-3 rounded-lg border border-white/15 transition-all duration-150 resize-none';
