...
export default function LeftSidebar({ onNotificationsToggle, onPostButtonClick, onSearchClick, notificationCount = 0 }: LeftSidebarProps) {
  ...
  return (
    <nav className="sticky top-0 h-screen w-[260px] shrink-0 z-30 border-r border-[#222222] px-[16px] py-[24px] hidden md:flex flex-col bg-[#111111]" role="navigation" aria-label="Main navigation">
      {/* Logo */}
          ...
          if (isDisabled) {
            return (
              ...
              >
                {content}
                {/* Tooltip on hover */}
                <div className="absolute left-full ml-2 top-1/2 transform -translate-y-1/2 bg-[#262626] border border-[#222222] text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none z-50 whitespace-nowrap">
                  Feature coming soon
                </div>
              </div>
            );
      ...
      {/* User Profile */}
      {connected && publicKey && (
        <div className="mt-[12px] p-[14px] rounded-xl bg-[#171717] border border-[#222222]">
