...
export default function ShoutoutModal({ isOpen, onClose, postContent, onConfirm, queueInfo }: ShoutoutModalProps) {
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget && !isProcessing) onClose(); }}>
      <div ref={modalRef} className="modal-content bg-[#1e1e1e] border border-white/[0.06] rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-3 p-6 border-t border-white/[0.06]">
          <button onClick={onClose} disabled={isProcessing} className="flex-1 px-5 py-3 bg-white/[0.08] border border-white/15 text-[#fafafa] font-semibold rounded-lg hover:bg-white/[0.12] duration-150 disabled:opacity-50 disabled:cursor-not-allowed">Cancel</button>
