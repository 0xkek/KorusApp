...
export function GamesPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return 'border-[var(--color-border-light)] bg-white/[0.06] text-[var(--color-text-secondary)]';
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 pb-24">
      ...
      {activeTab === 'lobby' && (
        ...
        <div className="space-y-0">
          {games.map((game) => (
            <div
              key={game.id}
              className="border-b border-[var(--color-border-light)] bg-white/[0.04]"
            >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {game.status === 'waiting' && publicKey?.toBase58() === game.player1 && (
                        <button
                          ...
                          className="bg-red-600 text-[var(--color-text)] font-bold px-3 py-1 rounded-full hover:bg-red-700 duration-150 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                      ...
                      {game.status === 'active' && (publicKey?.toBase58() === game.player1 || publicKey?.toBase58() === game.player2) && expandedGameId === game.id && (
                        <button
                          ...
                          className="bg-[#262626] text-[var(--color-text)] font-bold px-3 py-1 rounded-full hover:bg-[#303030] duration-150 text-xs"
                        >
                          Collapse
                        </button>
                      )}
                      {game.status === 'active' && (publicKey?.toBase58() === game.player1 || publicKey?.toBase58() === game.player2) && expandedGameId !== game.id && (
                        <button
                          ...
                          className="bg-green-600 text-[var(--color-text)] font-bold px-3 py-1 rounded-full hover:bg-green-700 duration-150 text-xs"
                        >
                          Play →
                        </button>
                      )}
              ...
              {expandedGameId === game.id && (
                <div className="px-6 pb-3 bg-[var(--color-surface)] border-t border-[var(--color-border-light)]">
                  <div className="py-3">
      ...
      {activeTab === 'completed' && (
        <>
          {completedLoading ? (
            ...
            <div className="text-center py-20">
              <div className="text-6xl mb-4 opacity-60">🏆</div>
              <p className="text-[var(--color-text)] text-lg font-medium">No completed games yet</p>
              <p className="text-[var(--color-text-secondary)] text-sm mt-2">
            ...
            <div className="space-y-0">
              {completedGames.map((game) => {
                ...
                return (
                  <div
                    key={game.id}
                    className="border-b border-[var(--color-border-light)] bg-white/[0.04] hover:bg-white/[0.06] duration-150 cursor-pointer"
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2 text-sm flex-wrap">
                            ...
                            {wagerAmount > 0 && (
                              <>
                                ...
                                {!isDraw && (
                                  <span className={`font-semibold ${isUserWinner ? 'text-green-400' : isUserLoser ? 'text-red-400' : 'text-[var(--color-text-secondary)]'}`}>
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <button
                              ...
                              className="bg-[var(--color-surface)] border border-[var(--color-border-light)] text-[var(--color-text)] font-bold px-3 py-1 rounded-full hover:bg-[var(--color-surface)] duration-150 text-xs"
                            >
      ...
      {showCreateModal && typeof window !== 'undefined' && createPortal(
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-[var(--color-overlay-background)] backdrop-blur-sm"
          onClick={() => setShowCreateModal(false)}
        >
          <div
            className="bg-[var(--color-surface)] border border-[var(--color-border-light)] rounded-2xl shadow-2xl p-8 max-w-lg w-full"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-[var(--color-text)]">Create New Game</h2>
              <button
                onClick={() => setShowCreateModal(false)}
                disabled={isProcessing}
                aria-label="Close modal"
                className="w-9 h-9 rounded-full hover:bg-white/[0.08] text-neutral-400 hover:text-[var(--color-text)] duration-150 flex items-center justify-center disabled:opacity-50"
              >
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowCreateModal(false)}
                disabled={isProcessing}
                className="flex-1 bg-white/[0.06] border border-[var(--color-border-light)] text-[var(--color-text)] font-semibold py-3 rounded-lg hover:bg-white/[0.08] duration-150 disabled:opacity-50"
              >
                Cancel
              </button>
              <button
      ...
      {showCancelModal && gameToCancelData && typeof window !== 'undefined' && createPortal(
        <div
          className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-[var(--color-overlay-background)] backdrop-blur-sm transition-opacity duration-150 ${
            cancelModalClosing ? 'opacity-0' : 'opacity-100'
          }`}
          onClick={cancellingGame ? undefined : closeCancelModal}
        >
          <div
            className={`bg-[var(--color-surface)] border border-[var(--color-border-light)] rounded-2xl shadow-2xl p-8 max-w-md w-full duration-150 duration-150 ${
          ...
          >
            {/* Loading Overlay */}
            {cancellingGame && (
              <div className="absolute inset-0 bg-[var(--color-surface)]/95 backdrop-blur-sm rounded-2xl flex flex-col items-center justify-center z-10">
                <div className="w-12 h-12 border-4 border-korus-primary/30 border-t-korus-primary rounded-full animate-spin mb-4"></div>
                <p className="text-[var(--color-text)] font-semibold text-lg">{cancelStatus}</p>
                <p className="text-[var(--color-text-secondary)] text-sm mt-2">Please wait...</p>
              </div>
            )}

            {/* Modal Header */}
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-[var(--color-text)] mb-2">Cancel Game</h2>
            ...
            {gameToCancelData.hasWager && (
              <div className="mb-6 p-4 bg-korus-primary/10 border border-korus-primary/20 rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-[var(--color-text-secondary)] text-sm">Refund Amount</span>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-white/[0.06] border border-[var(--color-border-light)] text-[var(--color-text)] font-semibold py-3 rounded-lg hover:bg-white/[0.08] duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Keep Game
              </button>
              <button
                ...
                disabled={!!cancellingGame}
                className="flex-1 bg-red-600 text-[var(--color-text)] font-bold py-3 rounded-lg hover:bg-red-700 duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
              >
