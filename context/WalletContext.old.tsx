...
export const WalletProvider: React.FC<WalletProviderProps> = ({ children }) => {
  ...
  const checkExistingWallet = async () => {
    await withErrorHandling(async () => {
      ...
      if (storedNFTAvatar) {
        try {
          logger.log('Found stored NFT avatar:', storedNFTAvatar);
          ...
          logger.log('Stored value was:', storedNFTAvatar);
        }
      } else {
        logger.log('No stored NFT avatar found');
      }
  ...
  const createNewWallet = async (): Promise<boolean> => {
    try {
      ...
      try {
        ...
        // For mock mode, generate a mock token
        if (!authResult.token && authResult.user) {
          const mockToken = `mock-token-${publicKey}-${Date.now()}`;
          await SecureStore.setItemAsync('korus_auth_token', mockToken);
          logger.log('Mock token generated for testing');
        }
  ...
  const setSelectedNFTAvatar = async (nft: NFTAvatar | null) => {
    try {
      ...
      if (nft) {
        // Just save the ID for now to test
        const simpleData = nft.id || 'test';
        logger.log('Saving simple NFT id:', simpleData);
        
        await SecureStore.setItemAsync(NFT_AVATAR_KEY, simpleData);
        
        // Verify it was saved
        const verify = await SecureStore.getItemAsync(NFT_AVATAR_KEY);
        logger.log('Verification - NFT avatar in storage:', verify);
        
        if (verify) {
          // Now try saving the full object
          ...
          logger.error('SecureStore is not working properly!');
        }
      } else {
        await SecureStore.deleteItemAsync(NFT_AVATAR_KEY);
        setSelectedNFTAvatarState(null);
