...
// Key generator: use wallet address for authenticated users, IP for anonymous
const walletKeyGenerator = (req: Request) =>
  (req as AuthRequest).userWallet || req.ip || 'unknown';
...
export const createPostLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    logger.debug(`Rate limit hit for wallet: ${(req as AuthRequest).userWallet || req.ip}`);
    res.status(429).json({
...
export const apiLimiter = rateLimit({
  ...
  max: process.env.NODE_ENV === 'development' ? 100000 : 500, // Extremely high limit for development
...
export const interactionLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 60, // Limit each wallet to 60 interactions per minute
  keyGenerator: walletKeyGenerator,
